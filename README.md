...
El [radar de disponibilidad estimada](Docs/architecture/radar-probabilidad.md) propone un piloto sin credenciales de empleado: combinar cupos comerciales por clase, pruebas de cotización para grupos, precio comparable y evolución diaria para indicar si **hay evidencia comercial de más de X plazas vendibles**. Incluye el plan de obtención y validación de una probabilidad numérica; la implementación pura de la señal está en `src/domain/market-signal.js`. Sin datos de producción ni resultados reales calibrados, el dashboard no presenta porcentajes.

El comando `node scripts/probe-amadeus.mjs UX1234 2026-10-15 MAD PMI 5` permite probar un vuelo concreto con credenciales de una **aplicación Amadeus** en el entorno del servidor (`AMADEUS_CLIENT_ID`, `AMADEUS_CLIENT_SECRET`, opcionalmente `AMADEUS_ENV=production`). Devuelve JSON y distingue ausencia de cobertura de cupo cero. No hay claves ni consultas reales en el repositorio.
