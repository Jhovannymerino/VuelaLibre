...
const [flightNumber, date, origin, destination, rawThreshold = "5"] = process.argv.slice(2);
...
if (
  ...
  process.stderr.write("Uso: node scripts/probe-amadeus.mjs UX1234 AAAA-MM-DD MAD PMI [X]\n");
  process.exit(2);
}
...
async function requestJson(url, options) {
  ...
  if (!response.ok) throw new Error(`Amadeus respondió HTTP ${response.status}`);
...
try {
  ...
  const response = await requestJson(`${base}/v1/shopping/availability/flight-availabilities`, {
    method: "POST",
    headers: {
  ...
  const flight = findAirEuropaAvailability(response, { origin, destination, date, flightNumber });
  const result = {
    observedAt: new Date().toISOString(),
    source: `amadeus-${process.env.AMADEUS_ENV === "production" ? "production" : "test-cache"}`,
    query: { flightNumber, date, origin, destination },
    flight,
    signal: assessMarketSignal({ threshold, fareClasses: flight?.fareClasses ?? [] }),
  };
